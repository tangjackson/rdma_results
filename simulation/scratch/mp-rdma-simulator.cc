...
uint32_t pfc_xoff = 1000, pfc_xon = 300;
...
void ReadFlowInput()
{
	if (flow_input.idx < flow_num)
	{
		...
		//std:cout<<flow_input.src<<" "<<flow_input.dst<<std::endl;
...
void ScheduleFlowInputs()
{
	...
	if (flow_input.idx < flow_num)
	{
		std::cout<<flow_input.src<<"-"<<flow_input.dst<<":"<<Seconds(flow_input.start_time)<<" "<<flow_input.start_time<<std::endl;
...
void get_pfc(FILE *fout, Ptr<MpQbbNetDevice> dev, uint32_t type, uint32_t qIndex)
{
fprintf(fout, "%lu %u %u %u %u %u\n", Simulator::Now().GetTimeStep(), dev->GetNode()->GetId(), dev->GetNode()->GetNodeType(), dev->GetIfIndex(), type, qIndex);
}
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
			...
			else if (key.compare("ENABLE_PFC") == 0)
			{
				...
				conf >> v;
				enable_pfc = v;
				std::cout << "ENABLE_PFC\t\t\t" << (enable_pfc ? "Yes" : "No") << "\n";
			}
			...
			else if (key.compare("PFC_XOFF") == 0)
			{
				conf >> pfc_xoff;
				std::cout << "PFC_XOFF\t\t\t\t" << pfc_xoff << '\n';
			}
			else if (key.compare("PFC_XON") == 0)
			{
				conf >> pfc_xon;
				std::cout << "PFC_XON\t\t\t\t" << pfc_xon << '\n';
			}
	...
	for (uint32_t i = 0; i < node_num; i++)
	{
		...
		{ // is switch
			...
			if (enable_pfc)
			{
				sw->m_mmu->ConfigPfc(pfc_xoff, pfc_xon);
			}
			else
			{
				// Belt-and-suspenders disable: even if the switch attribute were not
				// propagated as expected, make the MMU thresholds unreachable.
				sw->m_mmu->ConfigPfc(0xFFFFFFFFu, 0xFFFFFFFEu);
			}
			std::cout << "SWITCH_PFC\t\t\t"
					  << "node=" << sw->GetId()
					  << " enabled=" << (enable_pfc ? "Yes" : "No")
					  << " xoff=" << (enable_pfc ? pfc_xoff : 0xFFFFFFFFu)
					  << " xon=" << (enable_pfc ? pfc_xon : 0xFFFFFFFEu)
					  << "\n";
	...
	{
		std::cerr << "failed to open trace output file: " << trace_output_file << std::endl;
		return 1;
	}
	// Keep trace records crash-resilient. These experiments often terminate
	// early under extreme PFC pressure, and buffered binary output can be lost.
	setvbuf(trace_output, NULL, _IONBF, 0);
	if (enable_trace)
