...
int main(int argc, char *argv[])
{
	...
	flowf >> flow_num;
	tracef >> trace_num;
	uint32_t sender_node, pfc_node, backup_queue;
	topof >> sender_node >> pfc_node >> backup_queue;

	
	printf("Sender Node [%d], pfc Node [%d]", sender_node, pfc_node);

	// new test, 2nd line is sender id
	...
	for (uint32_t i = 0; i < node_num; i++){
		if (n.Get(i)->GetNodeType() == 0){ // is server
			...
			// for multi-queue transport
			rdmaHw->sender_node = sender_node;
			rdmaHw->backup_queue = backup_queue;
	...
	//
	// Before simulation, lets print all nodes
	//


	Ptr<RdmaDriver> rdma_2=n.Get(sender_node)->GetObject<RdmaDriver>();
	Ptr<Node> node_1 = n.Get(pfc_node);
	node_1->AggregateObject(rdma_2);
