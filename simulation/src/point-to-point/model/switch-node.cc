...
int SwitchNode::GetOutDev(Ptr<const Packet> p, CustomHeader &ch){
	...
	if(nexthops.size()>1 && (uint32_t)ch.udp.dport == 150){
		idx=0;
	}else if (nexthops.size()>1 && (uint32_t)ch.udp.dport == 200){
		idx=1;
	}else{
		idx = 0;
	}
...
int SwitchNode::GetOutDev_dip(uint32_t dip){
	// look up entries
...
void SwitchNode::CheckAndSendPromote(){
	if(m_mmu->node_id==1 && m_mmu->CheckShouldPromote()){
		printf("CheckAndSendPromote\n");
		Ptr<RdmaDriver> sub=GetObject<RdmaDriver>();
		if(sub==NULL){
			printf("Fail to get node rdma driver\n");
		}
		//int idx = GetOutDev_dip(184549889);
		//Ptr<QbbNetDevice> device = DynamicCast<QbbNetDevice>(m_devices[idx]);
		//device->SendPromo(3);
		sub->m_rdma->shouldPromo=1;
	}
}
void SwitchNode::CheckAndSendDePromote(uint32_t inDev, uint32_t qIndex){
	//printf("CheckAndSendDePromote\n");
	if(m_mmu->CheckShouldDePromote(inDev, qIndex)){
		Ptr<QbbNetDevice> device = DynamicCast<QbbNetDevice>(m_devices[inDev]);
		device->SendDePromo(qIndex);
	}
}

void SwitchNode::SendToDev(Ptr<Packet>p, CustomHeader &ch){
	// [Feng] Here is the point need to be changed to monitor right port for PFC
	CheckAndSendPromote();
...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p){
	...
	if (qIndex != 0){
		...
		//CheckAndSendPfc(inDev, qIndex);
		//CheckAndSendDePromote(inDev, qIndex);
