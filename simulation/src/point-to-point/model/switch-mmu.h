...
public:
	...
	bool CheckShouldPromote();
	bool CheckShouldDePromote(uint32_t port, uint32_t qIndex);
	...
	uint32_t GetPfcXoff(uint32_t port);
	uint32_t GetPfcXon(uint32_t port);
	uint32_t GetSharedUsed(uint32_t port, uint32_t qIndex);
	uint32_t GetEgressUsed(uint32_t port, uint32_t qIndex);
	...
	void ConfigPfc(uint32_t xoff, uint32_t xon);
