...
namespace ns3 {
	...
	void SwitchMmu::UpdateIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		...
		if (new_bytes <= reserve){
			...
			if (new_bytes - reserve > thresh){
				...
				ingress_bytes[port][qIndex] += psize;
				//std::cout<<port<<" "<<qIndex<<" "<<psize<<" "<<new_bytes<<" "<<reserve<<std::endl;
	...
	bool SwitchMmu::CheckShouldPause(uint32_t port, uint32_t qIndex){
		...
		if(!paused[port][qIndex] && used > GetPfcXoff(port)){
			std::cout<<"Pause: "<<used<<std::endl;
			return true;
		}else{
			return false;
		}
	...
	bool SwitchMmu::CheckShouldResume(uint32_t port, uint32_t qIndex){
		...
		uint32_t used = ingress_bytes[port][qIndex];
		if(paused[port][qIndex] && used < GetPfcXon(port)){
			std::cout<<"Resume: "<<used<<std::endl;
			return true;
		}else{
			return false;
		}
	...
	bool SwitchMmu::CheckShouldPromote(){
		uint32_t egress_used = egress_bytes[4][3];
		if(egress_used >= 1000 && !alert[3]){
			printf("node %d, %d\n",node_id,egress_used);
			alert[3] = 1;
			return true;
		}else{
	...
	bool SwitchMmu::CheckShouldDePromote(uint32_t port, uint32_t qIndex){
		uint32_t egress_used = egress_bytes[4][3];
		if(alert[3] && egress_used <= 100){
		 	printf("depromo, used egress %d\n",egress_used);
		 	alert[3] = 0;
			return true;
		}else{
			return false;
	...
	uint32_t SwitchMmu::GetPfcThreshold(uint32_t port){
		...
		// printf("total_hdrm: %d ",total_hdrm);
		// printf("total_rsrv: %d ",total_rsrv);
		// printf("shared_used_bytes: %d\n",shared_used_bytes);
		return (buffer_size - total_hdrm - total_rsrv - shared_used_bytes) >> pfc_a_shift[port];

	}

	uint32_t SwitchMmu::GetPfcXoff(uint32_t port){
		// printf("buffer_size: %d ",buffer_size);
		// printf("total_hdrm: %d ",total_hdrm);
		// printf("total_rsrv: %d ",total_rsrv);
		// printf("shared_used_bytes: %d\n",shared_used_bytes);
	...
	uint32_t SwitchMmu::GetPfcXon(uint32_t port){
		// printf("buffer_size: %d ",buffer_size);
		// printf("total_hdrm: %d ",total_hdrm);
		// printf("total_rsrv: %d ",total_rsrv);
		// printf("shared_used_bytes: %d\n",shared_used_bytes);
	...
	uint32_t SwitchMmu::GetEgressUsed(uint32_t port, uint32_t qIndex){
		uint32_t used = egress_bytes[port][qIndex];
		return used;
	}
	...
	void SwitchMmu::ConfigNPort(uint32_t n_port){
		...
		for (uint32_t i = 1; i <= n_port; i++){
			...
			total_rsrv += reserve;
			//printf("port %d, reserve, %d, headroom %d\n", i, reserve,headroom[i]);
		}
	}
	void SwitchMmu::ConfigBufferSize(uint32_t size){
		buffer_size = size;
	}
	void SwitchMmu::ConfigPfc(uint32_t xoff, uint32_t xon){
		pfc_xoff = xoff;
		pfc_xon = xon;
	}
}
