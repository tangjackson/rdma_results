...
namespace ns3
{
	...
	void QbbNetDevice::SendPromo(uint32_t qIndex)
	{
		Ptr<Packet> p = Create<Packet>(0);
		Ipv4Header ipv4h; // Prepare IPv4 header
		ipv4h.SetProtocol(0x92); //use unassinged protocol number
		ipv4h.SetSource(m_node->GetObject<Ipv4>()->GetAddress(m_ifIndex, 0).GetLocal());
		ipv4h.SetDestination(Ipv4Address(184549889));
		ipv4h.SetPayloadSize(p->GetSize());
		ipv4h.SetTtl(10);
		ipv4h.SetIdentification(UniformVariable(0, 65536).GetValue());
		p->AddHeader(ipv4h);
	...
	void QbbNetDevice::SendDePromo(uint32_t qIndex)
	{
		Ptr<Packet> p = Create<Packet>(0);
		Ipv4Header ipv4h; // Prepare IPv4 header
		ipv4h.SetProtocol(0x93); //use unassinged protocol number
		ipv4h.SetSource(m_node->GetObject<Ipv4>()->GetAddress(m_ifIndex, 0).GetLocal());
		
		ipv4h.SetDestination(Ipv4Address(184549889));//Ipv4Address(0x0b000001 + ((2 / 256) * 0x00010000) + ((2 % 256) * 0x00000100))
		ipv4h.SetPayloadSize(p->GetSize());
		ipv4h.SetTtl(10);
		ipv4h.SetIdentification(UniformVariable(0, 65536).GetValue());
		p->AddHeader(ipv4h);
