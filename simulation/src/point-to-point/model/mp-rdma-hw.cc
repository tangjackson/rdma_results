...
#define PRINT_LOG 0
    ...
    {
        // printf("ReceiveACK()\n");
        Ptr<MpRdmaQueuePair> qp = GetQp(ch.sip, ch.ack.sport, ch.ack.pg);
        if (qp == NULL)
        {
            // A late ACK/NACK can arrive after the sender has already completed
            // and deleted the QP. Drop it instead of dereferencing NULL.
            return 0;
        }
