...
namespace ns3
{
	...
	void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish)
	{
		...
		//uint32_t node_ip = Ipv4Address(0x0b000001 + ((2 / 256) * 0x00010000) + ((2 % 256) * 0x00000100)).Get();
		//printf("%d,%d, %d, %d, %d\n",m_node->GetId(),sip.Get(), sport, pg,node_ip);
		m_qpMap[key] = qp;
	...
	int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch)
	{
		...
		if(qIndex==4){
			printf("[Receiver] promoed udp\n");
			qIndex=3;
		}
		Ptr<RdmaRxQueuePair> rxQp = GetRxQp(ch.dip, ch.sip, ch.udp.dport, ch.udp.sport, qIndex, true);
		if (ecnbits != 0)
		{
	...
	int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch)
	{
		...
		int i;
		if(qIndex==4){
			printf("[Sender] promoed ACK/NACK\n");
			qIndex=3;
		}
	...
	int RdmaHw::ReceivePromo()
	{
		printf("Promo Received: %d\n", m_node->GetId());
		Ptr<RdmaQueuePair> qp = NULL;
		auto it = m_qpMap.begin();
		qp=it->second;
		
		if (qp == NULL)
		{
			std::cout << "[promo] ERROR: "
					  << "node:" << m_node->GetId() << ' ' <<  " NIC cannot find the flow\n";
			return 0;
		}
		if(qp->sip.Get()!=184549889){
			uint32_t node_id = (qp->sip.Get() >> 8) & 0xffff;
			std::cout<<"wrong host received: "<<node_id<<" "<<m_node->GetId()<<std::endl;
			return 0;
		}else{
			std::cout<<"right host:" << qp->sip.Get()<<" " <<" " << qp->m_pg<<std::endl;
	...
	int RdmaHw::ReceiveDePromo(Ptr<Packet> p, CustomHeader &ch)
	{
		printf("DePromo Received: %d\n", m_node->GetId());
		Ptr<RdmaQueuePair> qp = NULL;
		auto it = m_qpMap.begin();
		qp=it->second;
		...
		if (qp == NULL)
		{
			std::cout << "[depromo] ERROR: "
					  << "node:" << m_node->GetId() << ' ' << " NIC cannot find the flow\n";
			return 0;
		}else{
			std::cout<< m_qpMap.size()<<" " << qp->dip<<" " << qp->dport<<" " << qp->m_pg<<std::endl;
		}
		qp->m_pg = 3;
		return 0;
	}
	int RdmaHw::Receive(Ptr<Packet> p, CustomHeader &ch)
	{
		...
		else if (ch.l3Prot == 0x92){
			//ReceivePromo(p, ch);
		}
		else if (ch.l3Prot == 0x93){
			//ReceiveDePromo(p, ch);
		}
		return 0;
	}
	...
	Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp)
	{
		...
		if(this->enable_mq && shouldPromo>0 && m_node->GetId()==this->sender_node){
			seqTs.SetPG(this->backup_queue);
		}else{
			seqTs.SetPG(qp->m_pg);
		}
